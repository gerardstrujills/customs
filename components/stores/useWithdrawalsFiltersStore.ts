import { WithdrawalFilters } from "@/gen/gql";
import { endOfToday, startOfToday, subDays } from "date-fns";
import { DateRange } from "react-day-picker";
import { create } from "zustand";

interface WithdrawalFiltersStore {
  ...
  setFilters: (filters: Partial<WithdrawalFilters>) => void;
  setSelectedFilters: (
    filters: Partial<{
      unitOfMeasurement?: string;
      materialType?: string;
  ...
  // Nueva propiedad para almacenar los datos crudos del withdrawal
  rawwithdrawalData: Array<{
    unitOfMeasurement: string;
    materialType: string;
    description?: string | null;
  }> | null;
  ...
  getCombinedFilters: () => WithdrawalFilters;
}

export const useWithdrawalFiltersStore = create<WithdrawalFiltersStore>((set, get) => ({
  rawwithdrawalData: null,
  setRawWithdrawalData: (withdrawalData) => {
    set({ rawwithdrawalData: withdrawalData });

    // Automáticamente actualizar los valores únicos cuando se setean los datos
    if (withdrawalData && withdrawalData.length > 0) {
      const uniqueUnits = Array.from(
        new Set(
          withdrawalData.map((item) => item.unitOfMeasurement).filter(Boolean)
        )
      ).sort();

      const uniqueMaterials = Array.from(
        new Set(withdrawalData.map((item) => item.materialType).filter(Boolean))
      ).sort();
  ...
  setUniqueFilterValues: (withdrawalData) => {
    if (!withdrawalData || withdrawalData.length === 0) {
      set({
        uniqueFilterValues: {
          unitOfMeasurement: [],
    ...
    const uniqueUnits = Array.from(
      new Set(withdrawalData.map((item) => item.unitOfMeasurement).filter(Boolean))
    ).sort();
  ...
  getCombinedFilters: () => {
    const state = get();
    const combinedFilters: WithdrawalFilters = {
      ...state.filters,
    };
